...
const draft = () => ({
  ...
  clues: {
    queries: ['q1', 'q2'],
    links: [
      { title: 'A', url: 'https://a.example/', fetchedAt: '2026-09-24' },
      { title: 'B', url: 'https://b.example/', fetchedAt: '2026-09-24' },
    ],
    how: ['h1'],
  },
})
...
describe('mergeDraft（#83）', () => {
  it('本文は自分のノートの後ろに足し、空の要素は捨てる', () => {
    ...
    mergeDraft(l, draft(), [])
    expect(l.blocks.map((b) => [b.by, b.md])).toEqual([
      ['me', '待っている間に書いた'],
      ['ai', '### 本文'],
      ['ai', '### 要点'],
    ])
  ...
  it('手を動かすは、まだ無いときだけ採用する', () => {
    ...
    expect(empty.tasks).toEqual([
      { text: '試す', checked: false },
      { text: '書く', checked: false },
    ])
  ...
  it('参考情報は重複を除いて足し、リンクは URL が同じものを足さない', () => {
    const l = newLesson('l', {
      clues: { queries: ['q1'], links: [{ title: '既にある A', url: 'https://a.example/', fetchedAt: '2026-09-01' }], how: ['h0'] },
    })
    mergeDraft(l, draft(), [])
