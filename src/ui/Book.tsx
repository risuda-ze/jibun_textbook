...
export function Book({ tb }: { tb: Textbook }) {
  ...
  const chapters = tb.chapters
    .map((c, ci) => ({
      c,
      ci,
      lessons: c.lessons.map((l, li) => ({ l, li, blocks: lessonOk(l) ? l.blocks.filter(blockOk) : [] })).filter((x) => x.blocks.length),
    }))
  ...
  return (
    <>
      <PageHead
        eyebrow="教科書（通読）"
        title={tb.title}
        lead="全レッスンを1冊につなげた、読み返すための表示です。書くのはレッスン側で行います。"
        actions={
          <>
      ...
      <div className="row">
        {chapters.map(({ c, ci }) => (
          <Button
            ...
            v="outline"
            sm
            onClick={() => document.getElementById(`bkch${c.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
          >
            {ci + 1}. {c.title}
          </Button>
        ))}
      ...
      <div className="row" style={{ justifyContent: 'space-between' }}>
        ...
        <label className="row sub" htmlFor="blameB">
          <input type="checkbox" id="blameB" checked={blame} onChange={(e) => setBlame(e.target.checked)} />
          書き手の印を出す
        </label>
      </div>
            ...
            {lessons.map(({ l, li, blocks: bs }) => (
              <div key={l.id}>
                <h3 className="lt">
                  ...
                  {l.title}
                  {l.done && <Pill tone="done">完了</Pill>}
                  {l.review && <Pill tone="review">再確認</Pill>}
                  <button type="button" className="linkbtn" onClick={() => openLesson(l.id)}>
                    このページに書く
                  </button>
                </h3>
                {bs.map((b) => (
      ...
      <p className="sub">
        自分のノートと修正 {blocks.filter(isMine).length} / AIの下書き {blocks.filter((b) => !isMine(b)).length} / 再確認の節{' '}
        {reviewCount(tb)}
      </p>
    </>
  )
