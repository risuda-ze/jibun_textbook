...
import { dropBroken, go, openRepair, openBook, putBook, removeBook, useApp } from '../store'
...
import { allLessons, currentLesson, isMine, lessonNo, reviewCount } from '../lib/status'
import { Meter, downloadBook } from './common'
import { Button, Card, PageHead, Pill } from './kit'

const daysSince = (iso?: string): number | null => (iso ? Math.floor((Date.now() - Date.parse(iso)) / 86400000) : null)
      ...
      {broken.length > 0 && (
        <Card stack tone="peach" aria-label="読めない教科書">
          <p>
            ...
            形式が壊れているか、古い版のデータです。「Help
            で直す」で版の移行と修復を試せます。生データを書き出して保管するか、消すこともできます。
          </p>
          <div className="row">
            {broken.map((b) => (
              <span className="row" key={b.key}>
                ...
                <Button
                  v="outline"
                  sm
                  onClick={() => downloadText(`${b.key.replace(/[^a-z0-9_-]/gi, '_')}.raw.json`, JSON.stringify(b.raw ?? null, null, 1))}
                >
                  生データを書き出す
                </Button>
                <Button v="outline" sm onClick={() => openRepair(b.key, b.raw)}>
                  Help で直す
                </Button>
                <Button
                  v="danger"
                  sm
                  onClick={() => {
                    if (confirm('この生データを端末から消しますか？')) dropBroken(b.key)
                  }}
                >
                  消去
                </Button>
      ...
      {books.length === 0 ? (
        ...
        <div className="shelf">
          {books.map((b) => {
            ...
            const hasMine = allLessons(b).some((l) => l.blocks.some(isMine))
            ...
            return (
              <Card stack key={b.id}>
                ...
                <div className="row card-actions">
                  <Button v="outline" sm onClick={() => downloadBook(b)}>
                    JSON書出
                  </Button>
                  <Button
                    v="danger"
                    sm
                    onClick={() => {
                      if (confirm(`「${b.title}」を端末から消しますか？`)) removeBook(b.id)
                    }}
                  >
                    消去
                  </Button>
                  <Button v="soft" onClick={() => openBook(b.id)}>
                    開く
                  </Button>
                </div>
                <p className="sub" style={{ fontSize: 13 }}>
                  {cur ? `続き: ${lessonNo(b, cur.id)} ${cur.title}` : 'すべて完了'}
                  {rv > 0 && ` ・再確認 ${rv}件`}
                  {warn && (
                    <>
                      <br />
                      <Pill tone="review">{d === null ? 'まだ一度も書き出していません' : `最後の書き出しから${d}日`}</Pill>
                    </>
                  )}
                </p>
              </Card>
