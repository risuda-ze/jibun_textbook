    ...
    <Card stack className="clues" aria-label="参考情報">
      ...
      <div>
        ...
        <div className="row" style={{ marginTop: 6 }}>
          {c.queries.length ? (
            c.queries.map((x) => (
              <a className="qchip" key={x} href={gq(x)} target="_blank" rel="noopener noreferrer">
                {x}
              </a>
            ))
          ) : (
            <span className="sub">まだありません。下から足せます。</span>
          )}
        </div>
      </div>
      <div>
            ...
            c.links.map((x) => (
              ...
              <li key={x.url}>
                {isHttpUrl(x.url) ? (
                  ...
                  <span title="http(s) 以外のURLはリンクにしません">{x.title}</span>
                )}
                {x.fetchedAt && <span className="sub"> （{x.fetchedAt} 時点）</span>}
              </li>
            ))
          ) : (
      ...
      {c.how.length > 0 && (
        <div>
          <div className="eyebrow">確かめ方</div>
          <ul className="plain">
            {c.how.map((h) => (
              <li key={h}>{h}</li>
            ))}
      ...
      <div className="row">
        ...
        <Button
          ...
          onClick={() => {
            if (!q.trim()) return
            updateLesson(tb.id, lesson.id, (l) => {
              l.clues.queries.push(q.trim())
            })
            setQ('')
          }}
        >
      ...
      <div className="row">
        ...
        <Button
          ...
          onClick={() => {
            const u = url.trim()
            if (!isHttpUrl(u)) return toast('http から始まるURLを入れてください')
            updateLesson(tb.id, lesson.id, (l) => {
              l.clues.links.push({ title: linkTitle.trim() || u, url: u, fetchedAt: new Date().toISOString().slice(0, 10) })
            })
            setUrl('')
            setLinkTitle('')
          }}
  ...
  if (!f)
    return (
      <Card className="empty">
        <p>節が選ばれていません。</p>
        <Button v="primary" onClick={() => go('road')}>
          ロードマップへ
        </Button>
      </Card>
    )
    ...
    <>
      <div className="pagehead">
        <div>
          <div className="eyebrow">
            {lessonNo(tb, l.id)}・{l.minutes}分{l.isTask && '・実践課題'}
          </div>
          <h1>{l.title}</h1>
          <div className="row" style={{ marginTop: 6 }}>
            <StatusChip lesson={l} />
          ...
          {l.materials.length > 0 && (
            <div className="sub" style={{ marginTop: 4 }}>
              元にした資料: {l.materials.join('、')}
            </div>
          )}
        </div>
        <div className="row">
          {/* 表示領域の切り替え（#53）。スマホ幅では CSS で隠す（元から1列で画面いっぱい） */}
          <Button v="ghost" className="widebtn" aria-pressed={wide} onClick={() => setWide(!wide)}>
            {wide ? '幅を戻す' : '広げる'}
          </Button>
          <Button v="ghost" onClick={() => go('road')}>
            {L.roadmap}
          </Button>
          {next && (
            <Button v="ghost" onClick={() => openLesson(next.id)}>
              次へ {lessonNo(tb, next.id)}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="lesson">
        <div className="stack">
          ...
          <div className="row">
            <label className="row sub" htmlFor="blameL">
              <input type="checkbox" id="blameL" checked={blame} onChange={(e) => setBlame(e.target.checked)} />
              書き手の印を出す
            </label>
            <span className="sub">
              AI = AIの下書き / 自 = 自分のノート。本文はクリックして直接書き換えられます。本文を選択すると引用できます。
            </span>
          </div>
          <div className={`blocks doc ${blame ? '' : 'noblame'}`}>
            ...
            {composerAt >= l.blocks.length ? (
              ...
              <button type="button" className="ins" onClick={() => setInsAt(null)}>
                ＋ ここに書く
              </button>
            )}
          </div>
        ...
        <aside className="rail">
          <Card as="div">
            <h2>この節の印</h2>
            <div className="marks">
              <label className={`mark done ${l.done ? 'on' : ''}`} htmlFor="markdone">
                <input
                  type="checkbox"
                  id="markdone"
                  checked={l.done}
              ...
              <label className={`mark review ${l.review ? 'on' : ''}`} htmlFor="markreview">
                <input
                  type="checkbox"
                  id="markreview"
                  checked={l.review}
            ...
            <p className="sub" style={{ fontSize: 12, marginTop: 8 }}>
              どちらも自分のタイミングで付けます。完了の節は「設計を直す」で変更されません。
            </p>
          </Card>
          <Card as="div">
            <h2>この節の中身</h2>
            <div className="meter" role="img" aria-label={`自分の言葉 ${pct}%`}>
              <i style={{ flex: pct, background: 'var(--st-me)' }} />
              <i style={{ flex: 100 - pct, background: 'var(--st-ai)' }} />
            </div>
            ...
            <div className="row" style={{ marginTop: 8 }}>
              ...
              <Button
                ...
                onClick={() => {
                  if (!task.trim()) return
                  updateLesson(tb.id, l.id, (d) => {
                    d.tasks.push({ text: task.trim(), checked: false })
                  })
                  setTask('')
                }}
              >
      ...
      <button
        ...
        onPointerDown={(e) => {
          ...
          setDraft({ ...draft, quote: p.text })
          setInsAt(i >= 0 ? i + 1 : null)
