...
ここから下はこのアプリ固有の取り決め。上の Style Reference が正で、ここはその割り当て方を書く。
実装は `src/styles.css`（トークンとクラス）と `src/ui/kit.tsx`（共通コンポーネント）。

## 方針

- 参照元はマーケティングサイトの文法。このアプリは道具なので、**表示サイズは控えめに使う**（display 54〜96px は使わない。ページ見出しは heading 40px、スマホ幅は 28px）
- 参照元のロゴ・名称・イラスト（Avatar Character Mark など）は使わない。借りるのは色・書体・余白・形の体系だけ
- テーマはライトのみ。濃紺（Midnight Ink）は「完了」の色面にだけ使う
- 和文は Noto Sans JP / Noto Serif JP を後ろに置く。NotionInter → Inter、Lyon Text → Source Serif 4

## 共通コンポーネント（`src/ui/kit.tsx`）

| 部品 | 対応する Components | 使い方 |
|---|---|---|
| `Button v="primary"` | Primary CTA Button | 青の塗り。**1画面に1つ**。その画面でいちばん進めたい操作だけ |
| `Button v="soft"` | Ghost CTA Button | 薄い青の地。主操作の次に大事な操作 |
| `Button v="ghost"` | Ghost Text Button | 地も枠もない。その他の操作 |
| `Button v="outline" sm` | Outlined Text Button | 角丸4pxの細枠。行やカードの中の小さな操作 |
| `Pill tone=…` | Pill Tag | 節の状態と印。下の「状態の色」を参照 |
| `Card` | White Feature Card | 白地・罫線 rgba(0,0,0,.08)・角丸12px・余白24px・影なし |
| `Card tone="sky\|marigold\|peach\|midnight"` | Accent / Dark Feature Card | 色面。**1画面に1つまで**。罫線なし |
| `PageHead` | Section Header | 小見出し（caption）＋見出し＋リード文（明朝18px） |
| `Segmented` | — | どれか1つを選ぶ切り替え |
...
|---|---|
| 本棚 | AIと新しく作る |
| つくる | 調べてコース設計を作る → 設計案が出たら「この設計で始める」に移る |
| ロードマップ | 続きから。「設計を直す」を開いている間は「この案を採用」も青 |
| レッスン | 書き込む |
| 教科書（通読） | なし（読む画面） |
...
| 意味 | 色 | 使う所 |
|---|---|---|
| 未作成 | 白地＋細い枠 | ピル、タイムラインの節 |
| AIの下書き | Sky Tint `#e6f3fe` / メーターは Sky Wash `#62aef0` | 同上 |
| 書き込みあり | Marigold `#ffb110` | 同上。書き手の印「自」は Saffron |
| 完了 | Midnight Ink `#02093a`＋白文字 | 同上 |
| 再確認 | Coral `#f64932`＋白文字 | ピル、タイムラインの点、現在地の線 |
| 注意（キー未設定など） | Peach `#f6d5b8` | ピル |
| 色面のカード | 設計を直す = Sky Tint / 古いファイルの警告 = Marigold | — |

## 書体の使い分け

- 本文・UI は sans。レッスン本文と通読は 16px / 行間 1.8
- 明朝（serif）は3か所だけ: ページのリード文、教科書の目的、本文からの引用。ラベルやボタンには使わない
- 見出しは負の字間（40px で -0.025em、22px で -0.011em）。本文の字間は既定のまま

## やらないこと

- カードに影を付けない（影はナビだけ）。グラデーションを使わない
- 青以外の色で塗ったボタンを作らない。色の変化はカードの地とピルで出す
- 12px を超える角丸を四角い要素に使わない
