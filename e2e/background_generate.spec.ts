...
test('ロードマップで生成を始めて教科書タブへ移り、戻ってきても生成は続いていて、終わると本文が入る', async ({ page }) => {
  ...
  await expect(busyButton(page)).toHaveCount(0, { timeout: 15_000 })
...
test('別の節を選び直しても生成は続く', async ({ page }) => {
  ...
  await page.locator('.lessonlist li > button').locator('visible=true').nth(1).click()
  await expect(page.getByRole('button', { name: L.generateLesson })).toBeDisabled()
  // 元の節に戻ると進行中の表示が続いている
  await page.locator('.lessonlist li > button').locator('visible=true').nth(0).click()
  await expect(busyButton(page)).toBeVisible()
  await expect(busyButton(page)).toHaveCount(0, { timeout: 15_000 })
})
