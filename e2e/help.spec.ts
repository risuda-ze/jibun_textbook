...
test('本棚から Help に行き、3章が見え、本棚に戻れる', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: '不具合を知らせる' })).toHaveAttribute('href', /github\.com\/risuda-ze\/jibun_textbook\/issues\/new\?.*labels=bug/)
  ...
  await expect(page.getByRole('button', { name: 'AIと新規作成' })).toBeVisible()
})

test('JSON 読込に失敗すると、失敗文の近くから「読み込めない場合、まずはこちら」へ行ける', async ({ page }) => {
  await page.goto('./')
  await page.locator('#importfile').setInputFiles({ name: 'broken.textbook.json', mimeType: 'application/json', buffer: Buffer.from('{ これは JSON ではない') })
